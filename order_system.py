def place_order(menu):
    ...
    while True:
        ...
        if keep_ordering.lower() == "n":
            ...
            prices_list = [item['Quantity'] * item["Price"] for item in order]
...
def print_itemized_receipt(receipt):
    ...
    # print('my receipt',receipt)

    # TODO: Loop through the items in the customer's receipt
    for reciept_item in receipt:
        # for receipt_item in receipt:
        # TODO Store the dictionary items as variables
        item_price = reciept_item['Price']
        item_name = reciept_item['Item name']
        item_quantity = reciept_item['Quantity']
